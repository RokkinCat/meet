defmodule Meet.AvailabilityTest do
  ...
  @schedule ICalendar.from_ics(
    ...
    BEGIN:VEVENT
    DTSTART:20510706T173000Z
    DTEND:20510706T182000Z
  ...
  describe "available_at?/4" do
    ...
    test "available the day before " do
      assert available_at?(@schedule, ~N[2051-07-05 09:00:00])
    end
    ...
    test "available the same day, before the event" do
      assert available_at?(@schedule, ~N[2051-07-06 09:00:00])
    end

    test "available the same day, after the event" do
      assert available_at?(@schedule, ~N[2051-07-06 16:00:00])
    end

    test "not available at the start of the event" do
      assert not available_at?(@schedule, ~N[2051-07-06 17:30:00])
    end

    test "not available at the end of the event" do
      assert !available_at?(@schedule, ~N[2051-07-06 18:20:00])
    end

    test "not available during the event" do
      assert not available_at?(@schedule, ~N[2051-07-06 18:00:00])
    end

    test "not available in the offset minutes before the event" do
      assert not available_at?(@schedule, ~N[2051-07-06 17:15:00])
    end

    test "not available in the offset minutes after the event" do
      assert not available_at?(@schedule, ~N[2051-07-06 18:30:00])
    end
  ...
  describe "available_times_on/4" do
    ...
    test "all times are available on a day with no events" do
      times = available_times_on(@schedule, ~N[2051-07-05 09:00:00])
      assert Enum.count(times) == 15
    end

    test "exclude times around event" do
      times = available_times_on(@schedule, ~N[2051-07-06 09:00:00])
